...
# Simulate: caller creates follow-up request with threadId
cat > .cccx/review/REVIEW_REQUEST.md <<FOLLOWUP
---
profile: dev-plan
subject: test plan follow-up
threadId: $THREAD_ID
timestamp: 2026-03-27T12:30:00Z
---

## Goal
Re-review after fixing edge case tests.
...
- Added edge case test for empty input (tests/empty.test.js)
- Test output: 5 passing, 0 failing
FOLLOWUP

# Verify threadId survived the round-trip
ROUNDTRIP_ID=$(grep "^threadId:" .cccx/review/REVIEW_REQUEST.md | head -1 | sed 's/threadId: *//')
if [ "$ROUNDTRIP_ID" = "thread-xyz-456" ]; then
    pass "threadId round-trip preserved"
else
    fail "threadId round-trip broken: got '$ROUNDTRIP_ID'"
fi

# Verify follow-up has Changes section
if grep -q "## Changes Since Last Review" .cccx/review/REVIEW_REQUEST.md; then
    pass "follow-up has Changes Since Last Review section"
else
